...
r = 128  # Number of POD modes 10, latent 128
...
case_Number = 4 # 1, 2, 3, 4
###==================== Load saved reconstructed wind fields =======================###
##===POD======###
if case_Number == 1:
    u_rec_combined = np.load(result_repository + "U_rec_n_s_per_plane_%d_POD_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r) )
    err_pct_combined = np.load(result_repository + "U_error_n_s_per_plane_%d_POD_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r))
    
##========POD-NN ========###
if case_Number == 2:
    u_rec_combined = np.load(result_repository + "U_rec_n_s_per_plane_%d_POD_NN_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r) )
    err_pct_combined = np.load(result_repository + "U_error_n_s_per_plane_%d_POD_NN_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r))

##========AE ========###
if case_Number == 3:
    u_rec_combined = np.load(result_repository + "U_rec_n_s_per_plane_%d_AE_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r) )
    err_pct_combined = np.load(result_repository + "U_error_n_s_per_plane_%d_AE_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r))
##========AE-NN ========###
if case_Number == 4:
    u_rec_combined = np.load(result_repository + "U_rec_n_s_per_plane_%d_AE_NN_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r) )
    err_pct_combined = np.load(result_repository + "U_error_n_s_per_plane_%d_AE_NN_latent_%d_test_direction_281.25_uniformGrid.npy" %(n_s_per_plane, r))
...
plt.rcParams.update({
    ...
    'axes.titlesize': 18,        # Subplot title font
    'xtick.labelsize': 16,       # X-axis tick font
    'ytick.labelsize': 16,       # Y-axis tick font
    'font.size': 16,             # Base font size
    'legend.fontsize': 18,       # Legend font
    'figure.titlesize': 18       # Global title font
})
...
out_fig_errors = os.path.join(save_folder, 'U_error_plot_case_%d_test_wind_direction_%.2f_uniformGrid.png' %(case_Number, float(test_direction)) )
